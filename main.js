...
// Console easter egg
console.log(`
🚀 Welcome to Piyush Bhadade's Portfolio!
=====================================
...
function animateCarousel() {
    ...
    cards.forEach((card, i) => {
        const theta = ((i * step) + angle) % 360;
        const rad = theta * Math.PI / 180;
        const y = Math.sin(rad) * 30; // reduced from 60 to 30
        const z = Math.cos(rad) * 80;
        card.style.transform = `translate(-50%, -50%) translateY(${y}px) translateZ(${z}px)`;
        card.style.zIndex = Math.round(100 + z);
        card.style.opacity = 0.5 + 0.5 * (z / 80);
    });
    requestAnimationFrame(animateCarousel);
}
if (carousel && cards.length > 1) animateCarousel();

lottie.loadAnimation({
    container: document.getElementById('android-lottie'),
    renderer: 'svg',
    loop: true,
    autoplay: true,
    path: 'android.json' // Make sure android.json is in your project root
});